...
def get_battery_usage_v4(files_found, report_folder, seeker, wrap_text):
    ...
    if usageentries > 0:
        ...
        report.add_script()
        data_headers = ('Timestamp','Application','Package Name','Hidden','Boot Timestamp','Timezone','Total Power','Consumed Power','% Of Consumed','Foreground Usage (Seconds)','Background Usage (Seconds)','Battery Level (%)','Battery Status') 
        data_list = []
        for row in all_rows:
